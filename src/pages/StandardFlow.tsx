...
export default function StandardFlow() {
  ...
  const { methodDisabled, bank: linkedBank } = useLinkedBankWithdrawalRules(allBanks, selectedBank, setMethod, setIntlWire);

  const handleAccountChange = useCallback((acct: Account) => {
    if (acct.type === 'RRSP') {
  ...
  const withdrawalCurrency: Currency = useMemo(() => {
    if (!linkedBank) return 'CAD';
    if (method === 'international_wire') return intlWire.currency;
    return getLinkedBankDepositCurrency(linkedBank);
  }, [linkedBank, method, intlWire.currency]);

  const methodSelectorFeeCurrency: Currency | null = useMemo(() => {
  ...
  const withdrawalAmountStepData = useMemo(
    () =>
      account && selectedBank && method
        ? getWithdrawalAmountStepData(account, withdrawalCurrency)
        : null,
    [account, selectedBank, method, withdrawalCurrency],
  );
  ...
  const triggersConversion =
    parsedAmount > singleCurrencyBalance && !exceedsAvailable && parsedAmount > 0;
  const fee =
    method === 'wire'
  ...
  const intlWireStepComplete =
    method !== 'international_wire' ||
    !!(intlWire.bankName?.trim() && intlWire.swiftCode?.trim() && signed);
          ...
          <div className="max-w-md text-center px-6">
            ...
            <p className="text-base text-qt-secondary leading-6 mb-2">
              ...
              <strong className="text-qt-primary">
                {formatCurrency(parsedAmount, withdrawalCurrency)}
              </strong>{' '}
          ...
          <div className="px-6 py-6">
            <div className="bg-white border border-qt-border rounded-lg divide-y divide-qt-border mb-6">
              ...
              <div className="flex items-center justify-between px-5 py-4 bg-qt-bg-3">
                ...
                <p className="font-semibold text-lg text-qt-green-dark">{formatCurrency(Math.max(0, netAmount), withdrawalCurrency)}</p>
              </div>
              {((withdrawalCurrency === 'CAD' && parsedAmount > 50000) || (withdrawalCurrency === 'USD' && parsedAmount > 25000)) && (
      ...
      <main className="flex-1">
        <div className="max-w-[680px] mx-auto w-full px-6 py-10">
          ...
          <div className="flex flex-col gap-8">
              ...
              <section className="flex flex-col gap-5">
                ...
                {selectedBank ? (
                  <MethodSelector
                    ...
                    currency={methodSelectorFeeCurrency}
            ...
            <WizardSection visible={!!method && method === 'international_wire' && !!selectedBank}>
              <section>
            ...
            <WizardSection visible={!!selectedBank && !!method && intlWireStepComplete}>
              <section>
                <CurrencyInput
                  label={`Withdrawal amount (${withdrawalCurrency})`}
                  value={amount}
                  onChange={setAmount}
                  error={exceedsAvailable ? `Amount exceeds available balance of ${formatCurrency(maxAmount, withdrawalCurrency)}` : undefined}
                />
                {!exceedsAvailable && withdrawalAmountStepData && (
                  <p className="text-xs text-qt-secondary mt-1">
                    Available: {formatCurrency(maxAmount, withdrawalCurrency)}
                  </p>
                )}
                {triggersConversion && (
                  <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3">
                    <p className="text-sm text-amber-800">
                      Your request exceeds your {withdrawalCurrency} balance. An automatic currency conversion will be applied to cover the difference.
                    </p>
                  </div>
                )}
              </section>
